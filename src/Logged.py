...
class Configuration(tk.Frame):
	...
	def __init__(self, parent, controller):
		...
		self.selectded_currency = tk.StringVar()
		currencies = self.get_available_currencies()

		select_currency_combo = ttk.Combobox(self, textvariable = self.selectded_currency, values =currencies )
		select_currency_combo.grid(column = 2, row = 3,sticky="w", padx = "10 0")
		print(select_currency_combo)
		# create a binding event to link selected currency with 
		select_currency_combo.bind("<<ComboboxSelected>>", self.set_currency)
		# amount to trade:
	...
	def set_currency(self,controller):
		global local_currency
		local_currency =self.selectded_currency.get()
		print('evento trigered', 'valor: ', local_currency)
		controller.selected_currency = local_currency
		print('controler: ', controller.selected_currency)
	...
	def get_user_funds(self,controller):
		...
		#retrieves a list. 
		# print(self.select_currency_combo)
	...
	def start_trading(self,controller):
		...
		# global selected
		# selected = controller.selected_currency
		Amount = self.Amount.get()
		selected = local_currency
